...
export const extensionRevisions = sqliteTable(
  ...
  (table) => [
    ...
    // with it, the subquery is an ordered seek on an already
    // status-filtered index. rowid needs no explicit column: every SQLite
    // index entry carries it, which is what the subquery's rowid
    // tie-break reads.
    index("idx_extension_revisions_reviewed")
      .on(table.extensionId, sql`${table.reviewedAt} desc`)
